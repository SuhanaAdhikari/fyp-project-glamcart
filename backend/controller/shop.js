...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    ...
    if (shopExists) return next(new ErrorHandler("Seller already exists", 400));
    ...
    res.status(201).json({
      success: true,
      seller,
      message: "Shop registration submitted. Wait for admin approval before logging in.",
    });
  })
);

// seller activation links are intentionally disabled
// ✅ Updated Shop Activation Route to match frontend
router.get("/activation/:token", catchAsyncErrors(async (req, res) => {
  res.status(410).json({
    success: false,
    message: "Seller activation links are no longer used. Register your shop and wait for admin approval.",
  });
}));
